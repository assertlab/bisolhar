...
export function ActivityLogs({ commits, pullRequests }) {
  ...
  const getPRBadge = (state, merged) => {
    if (merged) return { text: 'Merged', color: 'bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-200' };
    if (state === 'open') return { text: 'Open', color: 'bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-200' };
    return { text: 'Closed', color: 'bg-red-100 dark:bg-red-900/20 text-red-800 dark:text-red-200' };
  };
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Últimos Commits */}
        <div>
          <h4 className="text-md font-semibold text-gray-900 dark:text-white mb-4">Últimos Commits</h4>
        ...
        {/* Últimos PRs */}
        <div>
          <h4 className="text-md font-semibold text-gray-900 dark:text-white mb-4">Últimos Pull Requests</h4>
          <div className="space-y-3 max-h-96 overflow-y-auto">
